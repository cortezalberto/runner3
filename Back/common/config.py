...
class Settings:
    ...
    # CORS
    # SECURITY: Use specific origins in production, never use "*" in production
    _cors_origins = os.getenv("CORS_ORIGINS", "http://localhost:5173,http://localhost:49173,http://localhost:3000")
    CORS_ORIGINS: List[str] = [origin.strip() for origin in _cors_origins.split(",")]

    # Optional: Allow all origins (ONLY for development, NEVER in production)
    # Set CORS_ALLOW_ALL_ORIGINS=true to enable (insecure)
